...
# devtools::document()
#' applysemrep
#' Function to apply the semantic representation (or word embeddings) to ONE word from a matrix of semreps; and
#' return a vector with NA if word is not found.
...
#' Function unites several text variables and rows to one, where all text is transformed to lowercase and tokenized.
...
# devtools::document()
#' sortingLayers
#' This is a function that sorts out the embeddings (and is used again below for decontextualized words).
...
#' @param model Character string specifying pre-trained language model. Default 'bert-base-uncased'; options "bert-base-multilingual-uncased", "bert-base-multilingual-cased", "openai-gpt",
#' "gpt2", "ctrl", "transfo-xl-wt103", "xlnet-base-cased", "xlm-mlm-enfr-1024", "distilbert-base-uncased", "roberta-base", "xlm-roberta-base", "xlm-roberta-large", or "T5Model". See
...
#' @param pretrained_weights Advanced parameter submitted to HuggingFace interface to get models not yet officially incorporated into *text*. Default = NULL. For alternatives see https://huggingface.co/.
#' @param tokenizer_class Advanced parameter submitted to HuggingFace interface to get models not yet officially incorporated into *text*. Default = NULL. for alternatives see https://huggingface.co/.
#' @param model_class Advanced parameter submitted to HuggingFace interface to get models not yet officially incorporated into *text*. Default = NULL. for alternatives see https://huggingface.co/.
#' @param layers Specify the layers that should be extracted (default 11:12). It is more efficient to only extract the layers
#' that you need (e.g., 11:12). You can also extract all by setting this parameter to "all". Layer 0 is the decontextualized input layer (i.e., not comprising hidden states) and thus should normally not be used.
...
#' @return A tibble with tokens, column specifying layer and word embeddings. Note that layer 0 is the input embedding to the transformer, and should
...
#' @param layers The numbers of the layers to be aggregated (e.g., c(11:12) to aggregate the eleventh and twelfth).
#' Note that layer 0 is the input embedding to the transformer, and should normally not be used. Selecting 'all' thus removes layer 0.
#' @param aggregation Method to aggregate the dimensions of each selected layer (default "mean"; see also "min",  "max" and "CLS").
#' @param tokens_select Option to only select embeddings linked to specific tokens such as [CLS] and [SEP] (default NULL).
#' @param tokens_deselect Option to deselect embeddings linked to specific tokens such as [CLS] and [SEP] (default NULL).
...
#' @param x Tibble/dataframe with at least one character variable.
#' @param contexts Provide word embeddings based on word contexts (standard method; default = TRUE).
#' @param decontexts Provide word embeddings of single words as input (embeddings, e.g., used for plotting; default = TRUE).
...
#' @param pretrained_weights advanced parameter submitted to the HuggingFace interface to get models not yet officially incorporated into *text*. Default = NULL. for details see https://huggingface.co/.
#' @param tokenizer_class advanced parameter submitted to the HuggingFace interface to get models not yet officially incorporated into *text*. Default = NULL. for details see https://huggingface.co/.
#' @param model_class advanced parameter submitted to the HuggingFace interface to get models not yet officially incorporated into *text*. Default = NULL. for details see https://huggingface.co/.
#' @param context_layers Specify the layers that should be aggregated (default 11:12). Layer 0 is the decontextualized input layer (i.e., not comprising hidden states) and thus advised not to be used.
...
#' @param decontext_aggregation Method to aggregate the decontextualized layers (i.e., embeddings from single words; e.g., "mean", "min", "max or "[CLS]").
...
#' @param decontext_tokens_deselect option to deselect embeddings linked to specific tokens such as [CLS] and [SEP] for the decontext embeddings.
#' @return A tibble with tokens, a column for layer identifier and word embeddings. Note that layer 0 is the input embedding to the transformer
#' @examples
#'\dontrun{
#' x <- Language_based_assessment_data_8_10[1:2, 1:2]
#' #Example 1
