...
test_that("1. textPredict generates embeddings from text and 2. automatically codes implicit motives", {
  ...
  testthat::expect_equal(predictions$person_predictions$person_prob[40], -1.13, tolerance = 0.0001)
  ...
  testthat::expect_equal(sum(as.numeric(predictions$person_predictions$person_prob[10])), 0.187, tolerance = 0.0001)
  
  # story-level predictions
  testthat::expect_equal(sum(as.numeric(predictions$story_predictions$story_id[5])), 5, tolerance = 0.0001)
  testthat::expect_equal(sum(as.numeric(predictions$story_predictions$story_prob[5])), -2.09, tolerance = 0.0001)

  # dataset merging
  testthat::expect_equal(sum(as.numeric(predictions$dataset$.pred_0_1[40])), 0.9891365, tolerance = 0.0001)
  testthat::expect_equal(sum(as.numeric(predictions$dataset$person_prob_2[10])),0.187, tolerance = 0.0001)
  testthat::expect_equal(sum(as.numeric(predictions$dataset$story_prob_3[5])), -2.09, tolerance = 0.0001)
})
