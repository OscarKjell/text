...
SemanticTrainModelForPrediction <- function(x, y, nrFolds=10, methodTrain = "lm", methodCor = "pearson", dim1 = TRUE, ...) {
    #Getting random number that have been saved from Matlab randomization; not sure if this works though...
    ...
    ###################
      ...
      #Get strongest correlation, so that we can select the model that created it.
      cor <- max(as.vector(unlist(results_corr[,"estimate"])))
      #Select best model with strongest correlation by selecting which numeric position  the highest correlation is positioned in; and if several take the first as this uses fewest dimensions.     nr_model <- which(as.vector(unlist(results_corr[,"estimate"])) == max(as.vector(unlist(results_corr[,"estimate"]))))
      if (length(nr_model) > 1) {
        nr_model <- nr_model[1]
      }else{
        nr_model <- nr_model
...
################################################################################################################################################################
########### End of SemanticTModelForPrediction ###################################################################################################################
################################################################################################################################################################
