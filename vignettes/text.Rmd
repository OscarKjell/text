...
### ```textPlotData()``` and ```textPlotViz()```:Show your data by plotting statistically significant words 

### ```textPlotData()```:Pre-processes the word data 

### A One-dimensional word plot
```{r, eval = TRUE, warning=FALSE, message=FALSE}
library(text)
