...
text_outcome_data <- Language_based_assessment_data_8["satisfactiontexts"]
text_outcome_data
...
#' @export
textFineTuneDomain <- function(text_outcome_data,
                             model_name_or_path = "bert-base-uncased", # Also how to find my previously created one?
                             output_dir = "./runs",
                            validation_proportion = 0.10,
                            evaluation_proportion = 0.10,
  ...
  if(ncol(text_outcome_data)>1){
    stop("Please only input a text column")
  }


  colnames(text_outcome_data) <-  c("text")
...
#  }
  ...
  json_path1 <- paste0(text_path, "/mlm_args.json")
