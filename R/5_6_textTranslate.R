...
#' @param target_lang (string)  The desired language output. Might be required for multilingual models (will not have any effect for single pair translation models).
...
#' @param return_incorrect_results (boolean)  Many models are not created to be able to provide translation, so this setting
...
textTranslate <- function(x,
  ...
  for (i_variables in seq_len(length(data_character_variables))) {
    ...
    names(output1) <- paste0(source_lang, "_to_", target_lang,
                             "_",
                             x_name)
