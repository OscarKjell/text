...
# To install the python packages torch and transformers through R, run: 
library(reticulate)
install_miniconda()
...
After a new install/update of *text*, RStudio crashed (Abort session) when running functions that fetches word embeddings (i.e., ```textHuggingFace``` or ```textEmbed```). 
...
# Subsequently re-install torch and transformers by running: 
conda_install(envname = 'r-reticulate', c('torch', 'transformers==2.4.1'), pip = TRUE)
