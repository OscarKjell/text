...
*Text* uses the R-package [reticulate](https://rstudio.github.io/reticulate/) as an interface to [Python](https://www.python.org/), and the python packages [torch](https://pytorch.org/) and [transformers](https://huggingface.co/transformers/). So it's important to install both the text-package and a python environment with the text required python packages that the text-package can use.
</br></br>
...
The success of the installation is dependent on using python and package versions that work together. The installation of the text-package with text required python packages is tested on Linux, Mac OS, and Windows. The installation procedure and details can be seen at [github actions](https://github.com/OscarKjell/text/actions) (look at workflow runs called System specific installation NoPy).
...
Pyhon_version <- c("'3.9.0'", "'3.9.0'", "'3.9.0'",
                   "'3.8.10'", "'3.8.10'", "'3.8.10'",
                   "'3.7.0'", "'3.7.0'", "'3.6.13'"
                )

torch <- c("'torch==1.7.1'", "'torch==1.7.1'", "'torch==1.7.1'", 
           "'torch==1.7.1'", "'torch==1.7.1'", "'torch==1.7.1'", 
           "'torch==0.4.1'", "'torch==0.4.1'", "'torch==1.10'")

transformers <- c("'transformers==4.12.5'", "'transformers==4.12.5'", "'transformers==4.12.5'",
...
mini_conda_table <- tibble::tibble(OS, Pyhon_version, torch, transformers, Success)
